...
import { consultDataAssets, dataApiService } from '../../../services';
import DataDetailModal from '../Modals/DataDetailModal';
...
const DataCatalog: React.FC<DataCatalogProps> = ({ onSelect, onRegister }) => {
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      try {
        setLoading(true);
        const data = await dataApiService.getDataAssets();
        setAssets(data);
      } catch (error) {
  ...
  const handleSend = async (text: string = inputValue) => {
    ...
    try {
      ...
      // Filter recommendations based on IDs returned by AI
      ...
      <div className="flex-1 overflow-hidden relative">
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center text-gray-400">
             ...
             <span className="text-sm">加载数据资源中...</span>
          </div>
        ) : (
          <>
